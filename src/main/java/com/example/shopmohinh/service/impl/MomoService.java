...
public class MomoService {
    ...
    private static final String ACCESS_KEY = EnvConfig.get("MOMO_ACCESS_KEY");
    private static final String SECRET_KEY = EnvConfig.get("MOMO_SECRET_KEY");
    private static final String REDIRECT_URL = "http://localhost:4200/momo-return/";
    ...
    public String createPaymentRequest(String amount, String paymentId) {
        try {
            ...
            String orderInfo = "Thanh toán mua hàng tại shopmohinh";
            String extraData = "";
    ...
    private String updatePaymentStatus(String momoResponse, String orderId) throws JSONException {
        JSONObject responseJson = new JSONObject(momoResponse);
        int resultCode = responseJson.getInt("resultCode");
        String message = responseJson.getString("message");

        // Ví dụ: 0 = Thành công, các mã khác = thất bại
        if (resultCode == 0) {
            // Cập nhật đơn hàng thành "PAID"
            // paymentRepository.updateStatus(orderId, PaymentStatus.SUCCESS);
            return "{\"status\":\"success\",\"orderId\":\"" + orderId + "\"}";
        } else {
            // paymentRepository.updateStatus(orderId, PaymentStatus.FAIL);
